...
if [[ ! -f "$LIBOPUS_LIB_DIR/opus-debug.a" ]] && [[ ! -f "$LIBOPUS_LIB_DIR/opus-release.a" ]]; then
    echo "Error: libopus must be built before opusfile. Please run build_libopus_macos.sh first." >&2
    exit 1
fi

if [[ ! -f "$LIBOGG_LIB_DIR/ogg-debug.a" ]] && [[ ! -f "$LIBOGG_LIB_DIR/ogg-release.a" ]]; then
    echo "Error: libogg must be built before opusfile. Please run build_libogg_macos.sh first." >&2
    exit 1
fi
...
if(NOT TARGET Ogg::ogg)
  ...
  if(PkgConfig_FOUND)
    # Try to find ogg via pkg-config, but don't fail if not found
  ...
  # If pkg-config didn't work, use manual variables
  if(NOT TARGET Ogg::ogg)
    if(OGG_INCLUDE_DIR AND OGG_LIBRARY)
      ...
      message(FATAL_ERROR "Ogg library not found. Please set OGG_INCLUDE_DIR and OGG_LIBRARY, or install PkgConfig with the ogg package.")
    endif()
  endif()
endif()
EOF
...
if(NOT TARGET Opus::opus)
  find_package(PkgConfig QUIET)
  if(PkgConfig_FOUND)
    # Try to find opus via pkg-config, but don't fail if not found
  ...
  if(NOT TARGET Opus::opus)
    if(OPUS_INCLUDE_DIR AND OPUS_LIBRARY)
      ...
      message(FATAL_ERROR "Opus library not found. Please set OPUS_INCLUDE_DIR and OPUS_LIBRARY, or install PkgConfig with the opus package.")
    endif()
  endif()
endif()
EOF
